...
- `src/scripts/` — browser-only modules (`theme`, `image-processing`, `pdf-generation`, `pdf-rendering`, `tool-search`, `gizlet-request`). Imported from component `<script>` blocks, never from Astro frontmatter.
