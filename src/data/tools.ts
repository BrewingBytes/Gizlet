...
export const toolRegistry = [
  ...
  },
  {
    ...
    launchStatus: "available",
    agent: {
      input: "One file from the device, of any kind, and optionally the hash it is supposed to have. The expected hash needs no algorithm chosen: its length says which digest wrote it, and a whole checksum line with the file name still attached is accepted as it was copied.",
      output: "The file's SHA-256, SHA-512, SHA-384, SHA-1 and MD5, each copyable, with SHA-1 and MD5 labelled as broken for authenticity rather than left out. Given an expected hash, the answer is a match or no match against the digest that hash's length names, with both hex strings shown as evidence beneath it. A match is stated as proving the file matches that hash and nothing about where the hash came from.",
    },
  },
  {
    id: 29,
  ...
  },
  {
    ...
    launchStatus: "available",
    agent: {
      input: "A choice of UUID kind — version 4, 7, 1, 6, 3, 5, or the Nil and Max values — with a namespace and a name for the two name-based versions, a written style (canonical, braces as a GUID, no hyphens, or a URN), a letter case, and how many to make. It also reads a UUID pasted in, in any of those styles.",
      output: "The identifiers, generated in the browser from crypto.getRandomValues or crypto.randomUUID and never Math.random, copyable or downloadable as one per line. A pasted UUID is decoded to its version and variant, the time it embeds where it embeds one, and whether its node field is a real MAC address or a random one. Versions 2 and 8 are explained rather than generated, since neither can be produced honestly in a browser.",
    },
  },
  {
    id: 33,
  ...
  },
  {
    ...
    category: "archive",
    description: "Look inside a ZIP and pull out what you need.",
    keywords: [
      "unzip",
      "extract zip",
      "open zip",
    ...
    launchStatus: "available",
    agent: {
      input: "A ZIP archive from the device. Its index is read in the browser and shown as a tree of folders and files, each with its size and how it is compressed.",
      output: "The ticked files, unpacked in the browser: one file as itself, or several as a ZIP named after the archive they came from. Stored and deflated entries are read; an encrypted entry, or one using a compression method no browser has, is listed and marked rather than silently skipped, and a path written to unpack outside its folder is corrected.",
    },
  },
  {
    id: 35,
