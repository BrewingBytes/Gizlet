...
test("turns an image inside a flow, and hands on the swapped sides", async ({ page }) => {
  await page.goto("/flows/");
  ...
  const nextTool = page.getByLabel("Next compatible Gizlet");
  await nextTool.selectOption("rotate-flip-image");
  await page.getByRole("button", { name: "Add step" }).click();
  await expect(page.getByLabel("Rotate & Flip Image turn")).toHaveValue("rotate-right");
  await page.getByLabel("Rotate & Flip Image turn").selectOption("rotate-left");

  // The block after it is handed the turned picture, not the one chosen: a
  // resize to the sides it now has is the proof.
  await nextTool.selectOption("resize-image");
  await page.getByRole("button", { name: "Add step" }).click();
  await page.getByLabel("Resize Image width").fill("20");
  await page.getByLabel("Resize Image height").fill("40");
  await page.getByRole("button", { name: "Run flow" }).click();

  await expect(page.getByAltText("Final flow result")).toHaveJSProperty("naturalWidth", 20);
  await expect(page.getByAltText("Final flow result")).toHaveJSProperty("naturalHeight", 40);
  await expect(page.getByRole("link", { name: "Download image" })).toHaveAttribute(
    "download",
    "sideways-rotated-resized.png",
  );

  await page.getByRole("button", { name: "Copy recipe link" }).click();
  await expect(page).toHaveURL(/rotate-flip-image:t=rotate-left/);
});

test("puts an image on a canvas inside a flow, and hands on the canvas", async ({ page }) => {
  await page.goto("/flows/");
  ...
  await page.getByLabel("Next compatible Gizlet").selectOption("image-background");
  await page.getByRole("button", { name: "Add step" }).click();

  // A canvas the size of the picture would be a block that does nothing, so a
  // flow starts at the square a listing asks for.
  await expect(page.getByLabel("Image Background canvas width")).toHaveValue("1080");
  await expect(page.getByLabel("Image Background fit")).toHaveValue("contain");
  await page.getByLabel("Image Background canvas width").fill("600");
  await page.getByLabel("Image Background canvas height").fill("600");
  await page.getByRole("button", { name: "Run flow" }).click();

  // The output is the canvas, not the picture: a 40x20 image comes back square.
  await expect(page.getByAltText("Final flow result")).toHaveJSProperty("naturalWidth", 600);
  await expect(page.getByAltText("Final flow result")).toHaveJSProperty("naturalHeight", 600);
  await expect(page.getByRole("link", { name: "Download image" })).toHaveAttribute(
    "download",
    "wide-background.png",
  );
  ...
  await page.getByRole("button", { name: "Copy recipe link" }).click();
  await expect(page).toHaveURL(/image-background:f=contain,cw=600,ch=600/);
});
